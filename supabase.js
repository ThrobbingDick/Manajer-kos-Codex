...
if (!SB_URL || !SB_KEY) {
  throw new Error('Supabase env belum lengkap. Isi VITE_SUPABASE_URL dan VITE_SUPABASE_ANON_KEY terlebih dahulu.')
}
...
export const resizeAndUpload = async (file, folder) => {
  ...
  const resized = await new Promise((res, rej) => {
    ...
    reader.onload = e => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(blob => blob ? res(blob) : rej(new Error('Gagal memproses gambar')), 'image/jpeg', 0.8)
      }
      img.onerror = rej
  ...
  const filename = `${user.id}/${folder}/${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`
